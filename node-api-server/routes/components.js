...
router.get("/getSongs", function (req, res) {
  ...
  const songs = {
    ...
    李: {
      ...
      name: "低空飞行",
      url:
        "http://qiniudns.shaxin741.vip/%E5%B0%8F%E5%A4%B4%E8%8F%9C%20-%20%E4%BD%8E%E7%A9%BA%E9%A3%9E%E8%A1%8C%EF%BC%88%E7%BF%BB%E8%87%AA%20%E6%B2%88%E4%BB%A5%E8%AF%9A%EF%BC%89.flac",
    },
  };
  var response = { code: 200, name: 1, data: songs[name] };
...
router.get("/getTableData", function (req, res) {
  ...
  // 这块大概有问题 我自己想的方法
  // TODO
  let sqlSelect = sortOrder
    ? "SELECT * FROM userdata where name like ? ORDER BY " +
      sortName +
      " " +
      sortOrder +
      " limit  ? ,?"
  ...
  db.pool.query(
    sqlSelect,
    ["%" + name + "%", (pageNo - 1) * pageSize, pageSize],
    (err, results) => {
