<template>
  <view v-show="url" class="ksp-image-cutter">
    <canvas :style="{width: target.width + 'px', height: target.height + 'px'}" canvas-id="target" />
    <view class="body">
      <image v-if="url" class="image" :style="{left: image.left + 'px', top: image.top + 'px', width: image.width + 'px', height: image.height + 'px'}" :src="url" @load="imageLoad" />
      <view v-if="mask.show" class="mask" />
      <view class="plank" @touchstart="touchStart($event, 'plank')" @touchmove="touchMove" @touchend="touchEnd" @touchcancel="touchCancel">
        <view class="frame" :style="{left: frame.left + 'px', top: frame.top + 'px', width: frame.width + 'px', height: frame.height + 'px'}" @touchstart="touchStart($event, 'frame')" @touchstart.stop.prevent="touchHandle">
          <canvas v-if="mask.show" class="canvas" :style="{width: frame.width + 'px', height: frame.height + 'px'}" canvas-id="canvas" />
          <view class="rect" />
          <view class="line-one" />
          <view class="line-two" />
          <view class="line-three" />
          <view class="line-four" />
          <view class="frame-left" @touchstart="touchStart($event, 'left')" @touchstart.stop.prevent="touchHandle" />
          <view class="frame-right" @touchstart="touchStart($event, 'right')" @touchstart.stop.prevent="touchHandle" />
          <view class="frame-top" @touchstart="touchStart($event, 'top')" @touchstart.stop.prevent="touchHandle" />
          <view class="frame-bottom" @touchstart="touchStart($event, 'bottom')" @touchstart.stop.prevent="touchHandle" />
          <view class="frame-left-top" @touchstart="touchStart($event, 'left-top')" @touchstart.stop.prevent="touchHandle" />
          <view class="frame-left-bottom" @touchstart="touchStart($event, 'left-bottom')" @touchstart.stop.prevent="touchHandle" />
          <view class="frame-right-top" @touchstart="touchStart($event, 'right-top')" @touchstart.stop.prevent="touchHandle" />
          <view class="frame-right-bottom" @touchstart="touchStart($event, 'right-bottom')" @touchstart.stop.prevent="touchHandle" />
        </view>
      </view>
    </view>
    <view class="toolbar">
      <button class="btn-cancel" @tap="oncancle">返回</button>
      <button class="btn-ok" @tap="onok">确定</button>
    </view>
  </view>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.targetContext = document.getElementById('target')
  },
  methods: {
    imageLoad(ev) {
      this.mask.show = true
