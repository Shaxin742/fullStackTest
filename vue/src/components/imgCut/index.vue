    ...
    <div class="container g-popup-box">
      ...
      <div class="content">
        ...
        <div class="c-right">
          ...
          <el-button>{{ !imgURL ? '上传图片' : '重新上传' }}</el-button>
    ...
    onEnter() {
      if (this.imgURL) {
        if (this.returnType === 'url') {
          this.$emit('ok', this.$refs.$canvas.toDataURL()) // 返回链接
        } else if (this.returnType === 'file') {
          this.$emit('ok', this.base64ToFile(this.$refs.$canvas.toDataURL(), 'avatar.png')) // 返回文件
        }
      } else {
        // this.$message.error('请上传图片')
        // this.$message.error({ message: '请上传图片', duration: 7000, showClose: true })
        this.$message({
          message: 'wwwww',
          type: 'error'
        })
